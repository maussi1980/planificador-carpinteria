...
function simSchedule(activos,exc={}){
  ...
  return activos.map(j=>{
    ...
    return{...j,inicio,fin,totalH:totalPend(j),tarde:!!(entrega&&fin>entrega)};
  });
}
...
function exportarCSV(trabajos){
  ...
  trabajos.forEach(j=>{
    ...
    const h=j.etapas.reduce((s,e)=>s+num(e.horas),0);const hh=j.etapas.reduce((s,e)=>s+num(e.hechas),0);
...
export default function App(){
  ...
  function agregarMaterial(){
    const m=nuevoMaterial.trim();if(!m)return;
    if(!materiales.includes(m))setMaterialesExtra(x=>[...x,m]);
    setGastoForm(f=>({...f,desc:m}));setMostrarNuevoMat(false);setNuevoMaterial("");showToast("Material agregado");
  }
  ...
  function importarArchivo(e){
    ...
    reader.onload=ev=>{
      try{
        ...
        if(!data.trabajos||!Array.isArray(data.trabajos))throw new Error("formato");
        if(!confirm(`Este respaldo tiene ${data.trabajos.length} trabajo(s). ¿Reemplazar todos los datos actuales?`))return;
        setTrabajos(data.trabajos.map(j=>({...j,etapas:j.etapas.map(et=>({...et,hechas:et.hechas||0}))})));
        if(data.excepciones)setExcepciones(data.excepciones);
        if(data.materialesExtra)setMaterialesExtra(data.materialesExtra);
        showToast("Respaldo importado correctamente");
      }catch(err){alert("El archivo no es un respaldo válido. Usa un archivo .json exportado desde esta misma app.");}
    };
    ...
    schedule.forEach(j=>{
      ...
      j.etapas.forEach(e=>{
        ...
        while(h>0.001){
          ...
          else mapa[key].push({jobId:j.id,jobNombre:j.nombre,cliente:j.cliente,etapa:e.nombre,horasPend:pend(e),horas:parseFloat(usado.toFixed(2)),color:j.color,tarde:j.tarde});
  ...
  const finGlobal=useMemo(()=>{
    ...
    return{caja:tA+tF-tG,anticipos:tA,pagosF:tF,gastado:tG,porCobrar:pC,horasTotales:hT,iph:hT>0?Math.round((tA+tF)/hT):0,enRiesgo:activos.filter(j=>saldoJob(j)<0)};
  },[trabajos,activos]);
  const maxIph=useMemo(()=>Math.max(...trabajos.map(j=>ingresosPorHora(j)),1),[trabajos]);

  // preview insercion (FIX: declarado después de schedule, antes del return)
  useEffect(()=>{
    ...
    setInsertPreview({pos,imp:calcImpacto(schedule,s2.filter(j=>j.id!==-1)),sched2:s2});
  },[insertPos,form,tab,editando,activos,excepciones,schedule]);

  // ── Registro de avance: marca horas hechas en una etapa ───────────
  function registrarAvance(jobId,etapaNombre,horasHechas){
    setTrabajos(t=>t.map(j=>{
      if(j.id!==jobId)return j;
      return{...j,etapas:j.etapas.map(e=>e.nombre===etapaNombre?{...e,hechas:Math.min(num(e.horas),num(e.hechas)+num(horasHechas))}:e)};
    }));
    showToast("Avance registrado — plan recalculado");
  }
  function setHechasEtapa(jobId,etapaNombre,valor){
    setTrabajos(t=>t.map(j=>j.id!==jobId?j:{...j,etapas:j.etapas.map(e=>e.nombre===etapaNombre?{...e,hechas:Math.max(0,Math.min(num(e.horas),num(valor)))}:e)}));
  }
  ...
  function guardarHorasEtapa(jobId,etapaNombre,nuevasHoras){
    ...
    setEditHoras(null);showToast("Horas actualizadas");
  }
  ...
  // Modal registrar avance (botón global)
  function abrirAvance(){
    setModal({tipo:"avance",jobId:activos[0]?.id||"",etapa:"",horas:""});
  }
  ...
  function onDrop(){
    ...
    const s2=simSchedule(arr,excepciones);const imp=calcImpacto(schedule,s2);
  ...
  const saveJob=()=>{
    ...
    else{const pos=Math.min(insertPos,activos.length);const lista=[...activos];lista.splice(pos,0,{...form,id:Date.now(),archivado:false,posicion:pos});lista.forEach((j,i)=>j.posicion=i);setTrabajos([...lista,...archivados]);showToast("Trabajo agregado");}
  ...
  const ImpactoPanel=({imp,schedDes,dark})=>{
    ...
    if(retrasados.length===0&&enRiesgoNuevo.length===0)return <div style={cS.ok}>✅ Sin impacto. Todos los trabajos se entregan a tiempo.</div>;
    return <div>
      {!viable&&<div style={cS.peligro}><div style={{fontWeight:500,marginBottom:6}}>🚨 No es viable sin atrasar compromisos</div>
      ...
      {retrasados.length>0&&<div style={cS.aviso}><div style={{fontWeight:500,marginBottom:6}}>⚠️ Trabajos que se retrasan:</div>
        {retrasados.map(j=>{const dr=diasRetraso(j);const sj=schedDes&&schedDes.find(s=>s.id===j.id);return <div key={j.id} style={{fontSize:12,marginTop:3}}>· <strong>{j.nombre}</strong> — <strong>{dr} día{dr!==1?"s":""}</strong> más tarde{sj&&<span> → {fmtShort(sj.fin)}</span>}{j.fechaEntrega&&<span style={{color:sj&&sj.tarde?"#a3261d":"inherit"}}> (comprometido: {fmtShort(parseDate(j.fechaEntrega))})</span>}</div>;})}
      </div>}
    </div>;
  };

  const DiaCtrl=({fecha,d})=>{
    const dow=d.getDay();const normal=dow===0?0:dow===6?HORAS_SAB:HORAS_DIA;
    const actual=excepciones[fecha]!==undefined?excepciones[fecha]:normal;
    const modificado=excepciones[fecha]!==undefined;const esFinde=dow===0||dow===6;
    const [ed,setEd]=useState(false);const [val,setVal]=useState(actual);
    return(
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"4px 0",fontSize:12,flexWrap:"wrap"}}>
        ...
        <button style={{...S.btn,padding:"2px 10px",fontSize:11,color:actual>0?"#1D9E75":"var(--color-text-secondary)",borderColor:actual>0?"#1D9E75":"var(--color-border-secondary)"}}
          onClick={()=>{if(actual===0){setExcepcion(fecha,HORAS_DIA);setVal(HORAS_DIA);}else if(dow===0){setExcepcion(fecha,0);}else resetExcepcion(fecha);}}>
          {actual>0?`✓ ${actual}h activo`:"+ Activar día"}</button>
        {actual>0&&(ed?(
          <div style={{display:"flex",gap:4,alignItems:"center"}}>
            <input type="number" min={0.5} max={12} step={0.5} value={val} onChange={e=>setVal(e.target.value)} style={{width:52,fontSize:12}} autoFocus onKeyDown={e=>{if(e.key==="Enter"){setExcepcion(fecha,parseFloat(val)||actual);setEd(false);}if(e.key==="Escape")setEd(false);}}/>
            <span style={{fontSize:11}}>h</span>
            <button style={{...S.btnG,padding:"2px 8px",fontSize:11}} onClick={()=>{setExcepcion(fecha,parseFloat(val)||actual);setEd(false);}}>✓</button>
            <button style={{...S.btn,padding:"2px 8px",fontSize:11}} onClick={()=>setEd(false)}>✕</button>
          </div>
        ):(<button style={{...S.btn,padding:"2px 8px",fontSize:11}} onClick={()=>{setVal(actual);setEd(true);}}>✏️ {actual}h</button>))}
  ...
  // etapas disponibles del trabajo seleccionado en modal avance
  const etapasDelJob=jobId=>{const j=trabajos.find(x=>x.id===parseInt(jobId));return j?j.etapas:[];};
  ...
  <div style={{fontFamily:"var(--font-sans)",color:"var(--color-text-primary)",maxWidth:780,margin:"0 auto",padding:"1rem"}}>
    ...
    {tab==="trabajos"&&<div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button style={S.btn} onClick={abrirAvance}>⏱ Registrar avance</button>
      ...
      {schedule.map((j,idx)=>{
        const hechas=j.etapas.reduce((s,e)=>s+num(e.hechas),0);const plan=totalHoras(j);
        const pctAvance=plan>0?Math.round((hechas/plan)*100):0;
        return(
        <div key={j.id} draggable onDragStart={()=>{dragIdx.current=idx;}} onDragEnter={()=>{dragOver.current=idx;}} onDragOver={e=>e.preventDefault()} onDrop={onDrop} style={{...S.card,borderLeft:`3px solid ${j.color}`}}>
          ...
          <div style={{marginTop:8,display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={()=>togglePF(j.id)} style={{...S.btn,padding:"3px 10px",fontSize:12,color:j.pagoFinalRecibido?"var(--color-text-success)":"var(--color-text-secondary)"}}>{j.pagoFinalRecibido?"✅ Pago recibido":"⬜ Pago pendiente"}</button>
    ...
    {tab==="gantt"&&<div>
      ...
      <div style={{display:"flex",gap:16,fontSize:11,color:"var(--color-text-secondary)",marginBottom:12,flexWrap:"wrap"}}>
        <span><span style={{display:"inline-block",width:10,height:10,background:"var(--color-background-secondary)",borderRadius:2,marginRight:4,verticalAlign:"middle"}}/>Finde</span>
        <span><span style={{display:"inline-block",width:10,height:10,border:"0.5px dashed var(--color-border-tertiary)",borderRadius:2,marginRight:4,verticalAlign:"middle"}}/>Día libre</span>
        <span><span style={{display:"inline-block",width:10,height:10,background:"#1D9E7544",borderRadius:2,marginRight:4,verticalAlign:"middle"}}/>Día extra activado</span>
      </div>
      ...
      <div style={{overflowX:"auto"}}><div style={{minWidth:Math.max(totalDiasG*24+180,400)}}>
        <div style={{display:"flex",marginLeft:160,marginBottom:2}}>
          {Array.from({length:totalDiasG}).map((_,i)=>{const d=addDays(ganttStart,i);const key=isoDate(d);const dow=d.getDay();const esHoy=key===isoDate(new Date());const finde=dow===0||dow===6;const exA=excepciones[key]!==undefined&&excepciones[key]>0;
            return <div key={i} style={{width:24,flexShrink:0,fontSize:9,textAlign:"center",color:esHoy?"#1D9E75":"var(--color-text-secondary)",background:exA?"#1D9E7522":finde?"var(--color-background-secondary)":"transparent",fontWeight:esHoy?700:400,borderLeft:esHoy?"2px solid #1D9E75":"none",boxSizing:"border-box"}}>{d.getDate()}</div>;})}
        </div>
        {schedule.map(j=>{const offset=Math.round((j.inicio-ganttStart)/86400000);const dur=Math.round((j.fin-j.inicio)/86400000)+1;
          return(<div key={j.id} style={{display:"flex",alignItems:"center",marginBottom:4}}>
            ...
            <div style={{position:"relative",flex:1,height:20,display:"flex"}}>
              {Array.from({length:totalDiasG}).map((_,i)=>{const d=addDays(ganttStart,i);const key=isoDate(d);const dow=d.getDay();const finde=dow===0||dow===6;const exA=excepciones[key]!==undefined&&excepciones[key]>0;const enRango=i>=offset&&i<offset+dur;const libre=!diasOcupados.has(key)&&!finde&&!exA;let bg="transparent";if(enRango)bg=j.color;else if(exA)bg="#1D9E7522";else if(finde)bg="var(--color-background-secondary)";
                return <div key={i} style={{width:24,flexShrink:0,height:20,background:bg,border:libre?"0.5px dashed var(--color-border-tertiary)":"none",boxSizing:"border-box",borderRadius:(i===offset||i===offset+dur-1)?4:0}}/>;})}
              {j.fechaEntrega&&(()=>{const eOff=Math.round((parseDate(j.fechaEntrega)-ganttStart)/86400000);if(eOff<0||eOff>=totalDiasG)return null;return <div style={{position:"absolute",left:eOff*24+11,top:0,width:2,height:20,background:j.tarde?"#E24B4A":"#1D9E75",zIndex:2}}/>;})()}
            </div>
          </div>);})}
        <div style={{display:"flex",alignItems:"center",marginTop:6}}>
          <div style={{width:160,flexShrink:0,fontSize:11,color:"var(--color-text-secondary)",paddingRight:8}}>Disponibilidad</div>
          <div style={{display:"flex"}}>{Array.from({length:totalDiasG}).map((_,i)=>{const d=addDays(ganttStart,i);const key=isoDate(d);const dow=d.getDay();const finde=dow===0||dow===6;const exA=excepciones[key]!==undefined&&excepciones[key]>0;const libre=!diasOcupados.has(key)&&!finde&&!exA;
            return <div key={i} title={`${key}: ${horasDelDia(d,excepciones)}h`} style={{width:24,flexShrink:0,height:8,borderRadius:2,background:exA?"#1D9E7566":finde?"var(--color-background-secondary)":libre?"#1D9E7522":"transparent",border:libre?"0.5px dashed #1D9E7566":"none",boxSizing:"border-box"}}/>;})}</div>
        </div>
      </div></div>
    </div>}
    ...
    {/* AGENDA */}
    {tab==="agenda"&&<div>
      <p style={{fontSize:13,color:"var(--color-text-secondary)",marginBottom:12}}>✏️ horas = corregir estimado · ✓ = registrar avance real · Mover = posponer trabajo</p>
      ...
      {(()=>{
        ...
        if(diasAgenda.length>0){const dI=parseDate(diasAgenda[0]);const dF=parseDate(diasAgenda[diasAgenda.length-1]);let cur=new Date(dI);while(cur<=dF){const dow=cur.getDay();if(dow===0||dow===6)allDias.add(isoDate(cur));cur=addDays(cur,1);}}
        return [...allDias].sort().map(dia=>{
            ...
            <div key={dia} style={{...S.card,borderLeft:esHoy?"3px solid #1D9E75":esSab||esDom?"3px solid #BA7517":"3px solid var(--color-border-tertiary)"}}>
              <div style={{fontWeight:500,fontSize:14,marginBottom:8,display:"flex",justifyContent:"space-between"}}>
                <div>
                  ...
                  {modificado&&<span style={{background:"#1D9E7522",color:"#1D9E75",fontSize:11,padding:"2px 7px",borderRadius:4,marginRight:6}}>⚡ {capActual}h</span>}
                  {fmtLong(d)}
                </div>
              ...
              {items.map((it,i)=>{
                ...
                return(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:"0.5px solid var(--color-border-tertiary)",flexWrap:"wrap"}}>
                    ...
                    {enEdic?(
                      <div style={{display:"flex",gap:4,alignItems:"center",flexShrink:0}}>
                        <input type="number" min={0} step={0.5} defaultValue={it.horasPend} style={{width:58,fontSize:13,textAlign:"center"}} autoFocus id={`inp-${editKey}`} onKeyDown={e=>{if(e.key==="Enter")guardarHorasEtapa(it.jobId,it.etapa,e.target.value);if(e.key==="Escape")setEditHoras(null);}}/>
                      ...
                      <button title="Corregir horas estimadas" onClick={()=>setEditHoras({key:editKey,jobId:it.jobId,etapa:it.etapa})} style={{fontSize:13,fontWeight:500,minWidth:38,textAlign:"right",flexShrink:0,background:"none",border:"none",cursor:"pointer",color:"var(--color-text-primary)",borderBottom:"1px dashed var(--color-border-secondary)",padding:"0 2px"}}>✏️{it.horas}h</button>
                      <button title="Registrar que ya hice estas horas" onClick={()=>setModal({tipo:"avance",jobId:it.jobId,etapa:it.etapa,horas:it.horas})} style={{...S.btn,padding:"3px 8px",fontSize:12,flexShrink:0,color:"#1D9E75",borderColor:"#1D9E75"}}>✓ hecho</button>
                      <button title="Posponer este trabajo" onClick={()=>pedirMover(it.jobId,d)} style={{...S.btn,padding:"3px 10px",fontSize:12,flexShrink:0}}>Mover</button>
                    </>)}
                    {it.tarde&&<span style={{fontSize:11,color:"var(--color-text-danger)"}}>⚠️</span>}
                  </div>
                );
              })}
              {(esSab||esDom||items.length>0)&&<div style={{marginTop:8,paddingTop:6,borderTop:"0.5px solid var(--color-border-tertiary)"}}><DiaCtrl fecha={dia} d={d}/></div>}
            </div>
          );
        });
      })()}
    ...
    {tab==="finanzas"&&<div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10,marginBottom:16}}>
        {[["Caja disponible",money(finGlobal.caja),finGlobal.caja>=0?"var(--color-background-success)":"var(--color-background-danger)",finGlobal.caja>=0?"var(--color-text-success)":"var(--color-text-danger)"],
          ["Anticipos",money(finGlobal.anticipos),null,null],["Pagos finales",money(finGlobal.pagosF),null,null],["Total gastado",money(finGlobal.gastado),null,null],
          ["Por cobrar",money(finGlobal.porCobrar),"var(--color-background-warning)","var(--color-text-warning)"],
          ["Horas totales",finGlobal.horasTotales.toFixed(0)+"h",null,null],["Ingreso / hora",money(finGlobal.iph),null,"var(--color-text-success)"],
        ].map(([l,v,bg,tc])=>(<div key={l} style={{background:bg||"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"0.75rem",textAlign:"center"}}><div style={{fontSize:11,color:tc||"var(--color-text-secondary)",marginBottom:4}}>{l}</div><div style={{fontSize:16,fontWeight:500,color:tc||"var(--color-text-primary)"}}>{v}</div></div>))}
      </div>
      <div style={S.card}>
        ...
        <div style={{marginTop:10}}>
          {gastoForm.cat==="Materiales"?(<div>
            <label style={S.lbl}>Material</label>
            <select value={gastoForm.desc} onChange={e=>{if(e.target.value==="__nuevo__")setMostrarNuevoMat(true);else{setGastoForm(f=>({...f,desc:e.target.value}));setMostrarNuevoMat(false);}}} style={{width:"100%",marginBottom:8}}>
              <option value="">Selecciona material…</option>{materiales.filter(m=>m!=="Otro").map(m=><option key={m} value={m}>{m}</option>)}<option value="__nuevo__">+ Agregar material nuevo…</option>
            </select>
            {mostrarNuevoMat&&<div style={{display:"flex",gap:8,marginBottom:8}}><input value={nuevoMaterial} onChange={e=>setNuevoMaterial(e.target.value)} placeholder="Material nuevo" style={{flex:1}} autoFocus onKeyDown={e=>{if(e.key==="Enter")agregarMaterial();if(e.key==="Escape"){setMostrarNuevoMat(false);setNuevoMaterial("");}}}/><button style={{...S.btnG,padding:"6px 12px",fontSize:13}} onClick={agregarMaterial}>Agregar</button><button style={{...S.btn,padding:"6px 10px",fontSize:13}} onClick={()=>{setMostrarNuevoMat(false);setNuevoMaterial("");}}>✕</button></div>}
            <input value={gastoForm.nota} onChange={e=>setGastoForm(f=>({...f,nota:e.target.value}))} placeholder="Nota adicional (opcional)" style={{width:"100%",boxSizing:"border-box",fontSize:13}}/>
          </div>):(<div><label style={S.lbl}>Descripción</label><input value={gastoForm.desc} onChange={e=>setGastoForm(f=>({...f,desc:e.target.value}))} placeholder="Descripción del gasto" style={{width:"100%",boxSizing:"border-box"}}/></div>)}
        </div>
      </div>
      ...
      {trabajos.map(j=>{const gT=totalGastos(j);const mg=margen(j);const sb=saldoJob(j);const hT=totalHoras(j);const iph=ingresosPorHora(j);const pct=num(j.total)>0?Math.min(100,Math.round((gT/num(j.total))*100)):0;const iphPct=maxIph>0?Math.round((iph/maxIph)*100):0;
        return(<div key={j.id} style={{...S.card,borderLeft:`3px solid ${j.color}`,opacity:j.archivado?0.75:1}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
            <div><div style={{fontWeight:500}}>{j.nombre}{j.archivado&&<span style={{fontSize:11,color:"var(--color-text-secondary)"}}> · archivado</span>}</div><div style={{fontSize:12,color:"var(--color-text-secondary)"}}>{j.cliente}</div></div>
            <div style={{textAlign:"right",fontSize:13}}><div>Precio: <strong>{money(num(j.total))}</strong></div><div style={{color:mg>=0?"var(--color-text-success)":"var(--color-text-danger)"}}>Margen: <strong>{money(mg)} ({margenPct(j)}%)</strong></div></div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6,marginBottom:10,fontSize:11}}>
            {[["Precio",money(num(j.total)),null],["Anticipo",money(num(j.anticipo)),null],["Gastado",money(gT),null],["Saldo",money(sb),sb<0?"var(--color-text-danger)":"var(--color-text-success)"],["Horas",hT.toFixed(1)+"h",null]].map(([l,v,c])=>(<div key={l} style={{background:"var(--color-background-secondary)",borderRadius:6,padding:"5px 6px",textAlign:"center"}}><div style={{color:"var(--color-text-secondary)",marginBottom:2,fontSize:10}}>{l}</div><div style={{fontWeight:500,color:c||"var(--color-text-primary)",fontSize:12}}>{v}</div></div>))}
          </div>
          <div style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}><span style={{color:"var(--color-text-secondary)"}}>Ingreso por hora trabajada</span><span style={{fontWeight:500,color:iphPct>=70?"var(--color-text-success)":iphPct>=40?"var(--color-text-warning)":"var(--color-text-danger)"}}>{money(iph)}/h</span></div>
            <div style={{height:6,background:"var(--color-background-secondary)",borderRadius:3,overflow:"hidden"}}><div style={{width:iphPct+"%",height:"100%",background:iphPct>=70?"#1D9E75":iphPct>=40?"#BA7517":"#E24B4A"}}/></div>
          </div>
    ...
    {tab==="historial"&&<div>
      ...
      {archivados.map(j=>(<div key={j.id} style={{...S.card,borderLeft:`3px solid ${j.color}`,opacity:0.8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div><div style={{fontWeight:500}}>{j.nombre}</div><div style={{fontSize:13,color:"var(--color-text-secondary)"}}>{j.cliente}</div></div><div style={{display:"flex",gap:6}}><button style={S.btn} onClick={()=>restaurar(j.id)}>Restaurar</button><button style={S.btnR} onClick={()=>eliminar(j.id)}>Eliminar</button></div></div>
        <div style={{display:"flex",gap:12,marginTop:8,flexWrap:"wrap",fontSize:13}}>{j.fechaEntrega&&<span>📅 {fmtShort(parseDate(j.fechaEntrega))}</span>}<span>💰 {money(num(j.total))}</span><span style={{color:margen(j)>=0?"var(--color-text-success)":"var(--color-text-danger)"}}>Margen: {money(margen(j))} ({margenPct(j)}%)</span><span>⏱ {totalHoras(j).toFixed(0)}h · {money(ingresosPorHora(j))}/h</span></div>
      </div>))}
      {archivados.length>0&&<div style={{...S.card,background:"var(--color-background-secondary)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,fontSize:13}}>
          ...
          <div><div style={{color:"var(--color-text-secondary)",fontSize:11}}>Facturado</div><div style={{fontWeight:500}}>{money(archivados.reduce((s,j)=>s+num(j.total),0))}</div></div>
    ...
    {tab==="form"&&<div>
      ...
      {!editando&&activos.length>0&&<div style={S.card}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {[...Array(activos.length+1)].map((_,pos)=>{const label=pos===0?"Al inicio":pos===activos.length?"Al final":`Después de: ${activos[pos-1].nombre}`;const sel=insertPos===pos;
            ...
            return(<label key={pos} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",borderRadius:"var(--border-radius-md)",border:`1px solid ${sel?"#1D9E75":"var(--color-border-tertiary)"}`,background:sel?"#1D9E7511":"transparent",cursor:"pointer"}}><input type="radio" name="insertPos" checked={sel} onChange={()=>setInsertPos(pos)} style={{accentColor:"#1D9E75"}}/><span style={{flex:1,fontSize:13}}>{label}</span><span style={{fontSize:14}}>{icon}</span></label>);})}
        </div>
    ...
    {modal&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:"1rem"}}>
      <div style={{background:"#ffffff",color:"#111",border:"1px solid #ddd",borderRadius:12,padding:"1.5rem",width:"100%",maxWidth:480,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 8px 40px rgba(0,0,0,0.25)"}}>
        ...
        {modal.tipo==="avance"&&<>
          <div style={{fontWeight:500,fontSize:16,marginBottom:4,color:"#111"}}>⏱ Registrar avance real</div>
          <div style={{fontSize:13,color:"#666",marginBottom:14}}>Marca cuántas horas trabajaste de verdad. El plan hacia adelante se recalcula solo con lo que falta.</div>
          <div style={{marginBottom:12}}><label style={{fontSize:13,color:"#666",display:"block",marginBottom:4}}>Trabajo</label>
            <select value={modal.jobId} onChange={e=>setModal(m=>({...m,jobId:parseInt(e.target.value),etapa:""}))} style={{width:"100%",fontSize:14,padding:"8px",border:"1px solid #ccc",borderRadius:6}}>
              <option value="">Selecciona…</option>{activos.map(j=><option key={j.id} value={j.id}>{j.nombre}</option>)}
            </select></div>
          <div style={{marginBottom:12}}><label style={{fontSize:13,color:"#666",display:"block",marginBottom:4}}>Etapa</label>
            <select value={modal.etapa} onChange={e=>setModal(m=>({...m,etapa:e.target.value}))} style={{width:"100%",fontSize:14,padding:"8px",border:"1px solid #ccc",borderRadius:6}}>
              <option value="">Selecciona…</option>{etapasDelJob(modal.jobId).map(e=>{const p=pend(e);return <option key={e.nombre} value={e.nombre} disabled={p<=0}>{e.nombre} — {p>0?`${p}h pendientes`:"completada"}</option>;})}
            </select></div>
          <div style={{marginBottom:8}}><label style={{fontSize:13,color:"#666",display:"block",marginBottom:4}}>Horas trabajadas hoy</label>
            <input type="number" min={0} step={0.5} value={modal.horas} onChange={e=>setModal(m=>({...m,horas:e.target.value}))} placeholder="Ej: 2" style={{width:"100%",boxSizing:"border-box",fontSize:14,padding:"8px",border:"1px solid #ccc",borderRadius:6}}/>
          </div>
          {modal.etapa&&(()=>{const e=etapasDelJob(modal.jobId).find(x=>x.nombre===modal.etapa);if(!e)return null;const p=pend(e);return <div style={{fontSize:12,color:"#666",marginBottom:8}}>Pendiente actual: {p}h{num(modal.horas)>p&&<span style={{color:"#a3261d"}}> · Solo se registrarán {p}h (lo que faltaba)</span>}</div>;})()}
          <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:12}}><button style={{background:"transparent",border:"1px solid #ccc",borderRadius:6,padding:"6px 14px",cursor:"pointer",fontSize:13,color:"#333"}} onClick={()=>setModal(null)}>Cancelar</button><button style={S.btnG} onClick={()=>{if(!modal.jobId||!modal.etapa||!modal.horas)return alert("Completa todos los campos.");registrarAvance(modal.jobId,modal.etapa,modal.horas);setModal(null);}}>Registrar avance</button></div>
        </>}
      </div>
    </div>}
  </div>
